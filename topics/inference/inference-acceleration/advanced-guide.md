...
- **直觉/作用**：每生成一个 token 都要"看"之前所有位置，计算量随长度线性增长。
...
- **变量说明**：$L$ 为层数；$h$ 为注意力头数；$d_{head}$ 为每头维度；$s$ 为序列长度；bytes 为每个数值的字节数。
- **直觉/作用**：序列越长缓存越大，是长上下文推理的主要内存瓶颈；2 表示 K 和 V 两份缓存。
...
- **变量说明**：$Q, K, V$ 为查询、键、值矩阵；$d$ 为键维度；$\sqrt{d}$ 为缩放因子。
- **直觉/作用**：核心注意力公式，但需要存储 $N \times N$ 的注意力矩阵，内存开销大。
